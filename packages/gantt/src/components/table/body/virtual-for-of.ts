...
export class VirtualForOf<T> implements CdkVirtualScrollRepeater<T>, CollectionViewer, DoCheck, OnDestroy {
    ...
    @Input()
    get virtualForOf(): DataSource<T> | Observable<T[]> | NgIterable<T> | null | undefined {
        return this._cdkVirtualForOf;
    }
    set virtualForOf(value: DataSource<T> | Observable<T[]> | NgIterable<T> | null | undefined) {
        this._cdkVirtualForOf = value;
        if (isDataSource(value)) {
            this._dataSourceChanges.next(value);
        } else {
            // If value is an an NgIterable, convert it to an array.
            this._dataSourceChanges.next(new ArrayDataSource<T>(isObservable(value) ? value : Array.from(value || [])));
        }
    }
     ...
     */
    @Input()
    get virtualForTrackBy(): TrackByFunction<T> | undefined {
        return this._cdkVirtualForTrackBy;
    }
    set virtualForTrackBy(fn: TrackByFunction<T> | undefined) {
    ...
    measureRangeSize(range: ListRange, orientation: 'horizontal' | 'vertical'): number {
        ...
        for (let i = 0; i < rangeLen; i++) {
            const view = this._viewContainerRef.get(i + renderedStartIndex) as EmbeddedViewRef<CdkVirtualForOfContext<T>> | null;
    ...
    private _onRenderedDataChange() {
        ...
        if (!this._differ) {
            ...
            this._differ = this._differs.find(this._renderedItems).create((index, item) => {
                return this.virtualForTrackBy ? this.virtualForTrackBy(index, item) : item;
            });
        }
        this._needsUpdate = true;
    }
    ...
    private _applyChanges(changes: IterableChanges<T>) {
        ...
        changes.forEachIdentityChange((record: IterableChangeRecord<T>) => {
            const view = this._viewContainerRef.get(record.currentIndex!) as EmbeddedViewRef<CdkVirtualForOfContext<T>>;
    ...
    private _getEmbeddedViewArgs(record: IterableChangeRecord<T>, index: number): _ViewRepeaterItemInsertArgs<CdkVirtualForOfContext<T>> {
